...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtextextended('run_daily_automations:' || v_run_date::text, 0)
  );
  ...
  IF v_run_id IS NULL THEN
    ...
    SET status = 'running', started_at = now(), completed_at = NULL,
        error_message = NULL, processed_count = 0
    WHERE id = v_run_id;
  END IF;

  BEGIN
    WITH due AS (
      SELECT m.id AS member_id, m.full_name, m.church_id, m.date_of_birth
    ...
    picked AS (
      SELECT d.member_id, d.full_name, d.church_id, a2.message_template
      FROM due d
      JOIN LATERAL (
        SELECT a2.message_template
        FROM public.automations a2
        WHERE a2.church_id IS NOT DISTINCT FROM d.church_id
          AND a2.type = 'birthday' AND a2.is_enabled = true
        ORDER BY a2.created_at DESC
        LIMIT 1
      ) a2 ON true
    ),
    to_send AS (
      SELECT p.member_id, p.church_id,
        REPLACE(REPLACE(p.message_template, '{{name}}', p.full_name), '{{pastor}}', v_pastor) AS content,
        'Birthday Ã°Å¸Å½â€°'::text AS title, 'birthday'::text AS automation_type
      FROM picked p
    ...
    inserted_announcements AS (
      ...
      SELECT title, content, true, church_id FROM to_send
      RETURNING id
    ),
    inserted_logs AS (
      INSERT INTO public.automation_logs (member_id, automation_type, message)
      SELECT member_id, automation_type, content FROM to_send
      RETURNING id
    )
    SELECT count(*)::integer INTO v_batch_count FROM inserted_logs;

    v_processed_count := v_processed_count + coalesce(v_batch_count, 0);

    WITH due AS (
      SELECT m.id AS member_id, m.full_name, m.church_id, m.wedding_date
      FROM public.members m
    ...
    picked AS (
      ...
      JOIN LATERAL (
        ...
        FROM public.automations a2
        WHERE a2.church_id IS NOT DISTINCT FROM d.church_id
          AND a2.type = 'anniversary' AND a2.is_enabled = true
        ORDER BY a2.created_at DESC
        LIMIT 1
      ) a2 ON true
    ),
    to_send AS (
      SELECT p.member_id, p.church_id,
        REPLACE(REPLACE(p.message_template, '{{name}}', p.full_name), '{{pastor}}', v_pastor) AS content,
        'Anniversary Ã°Å¸â€™Â'::text AS title, 'anniversary'::text AS automation_type
      FROM picked p
      WHERE p.message_template IS NOT NULL
        AND NOT EXISTS (
          SELECT 1 FROM public.automation_logs l
    ...
    SET status = 'completed', completed_at = now(),
        processed_count = v_processed_count, error_message = NULL
    WHERE id = v_run_id;
    ...
    SET
      ...
      updated_at = now()
    WHERE job_name = 'Daily Automations';
  EXCEPTION WHEN OTHERS THEN
    ...
    SET status = 'failed', completed_at = now(),
        processed_count = v_processed_count, error_message = SQLERRM
    WHERE id = v_run_id;
    ...
    WHERE NOT EXISTS (
      ...
      WHERE sa.alert_type = 'automation_failure'
        AND sa.resolved = false
        AND sa.created_at::date = v_run_date
    );
...
REVOKE ALL ON FUNCTION public.run_daily_automations() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.run_daily_automations() FROM anon;
REVOKE ALL ON FUNCTION public.run_daily_automations() FROM authenticated;
