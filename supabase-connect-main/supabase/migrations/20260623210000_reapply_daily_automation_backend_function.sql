-- Reapply the corrected backend runner under a new migration version.

DROP FUNCTION IF EXISTS public.run_daily_automations();

CREATE FUNCTION public.run_daily_automations()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  BEGIN
    ...
    to_send AS (
      SELECT p.member_id, p.church_id,
        ...
        'Birthday 🎉'::text AS title, 'birthday'::text AS automation_type
      FROM picked p
    ...
    to_send AS (
      SELECT p.member_id, p.church_id,
        REPLACE(REPLACE(p.message_template, '{{name}}', p.full_name), '{{pastor}}', v_pastor) AS content,
        'Anniversary 💍'::text AS title, 'anniversary'::text AS automation_type
      FROM picked p
      WHERE p.message_template IS NOT NULL
        AND NOT EXISTS (
          SELECT 1 FROM public.automation_logs l
