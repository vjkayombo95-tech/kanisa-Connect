...
-- The frontend and several later migrations use these legacy relation names.
-- Keep them as first-class compatibility tables on a fresh database instead of
-- assuming production-only schema drift created them.
CREATE TABLE IF NOT EXISTS public.member_communities (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  community_id uuid NOT NULL REFERENCES public.communities(id) ON DELETE CASCADE,
  member_id uuid NOT NULL REFERENCES public.members(id) ON DELETE CASCADE,
  joined_at timestamptz NOT NULL DEFAULT now(),
