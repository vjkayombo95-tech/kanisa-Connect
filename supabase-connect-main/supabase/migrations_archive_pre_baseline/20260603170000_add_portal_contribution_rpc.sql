...
begin
  ...
  if not public.is_church_member(auth.uid(), _church_id) then
  ...
  if _category_id is not null and not exists (
    ...
    where c.id = _category_id
      and c.church_id = _church_id
  ) then
    return jsonb_build_object('success', false, 'error', 'Contribution category does not belong to this church');
  end if;
