-- Chat access rules support both historic Swahili leadership columns and the
-- newer English aliases. The aliases must exist before SQL functions/policies
-- below are parsed on an empty project.
alter table public.communities add column if not exists chairperson_id uuid references public.members(id) on delete set null;
alter table public.communities add column if not exists vice_chairperson_id uuid references public.members(id) on delete set null;
alter table public.communities add column if not exists treasurer_id uuid references public.members(id) on delete set null;
alter table public.communities add column if not exists secretary_id uuid references public.members(id) on delete set null;
      ...
      and (
        ...
        or c.church_id in (
          ...
          where ur.user_id = auth.uid()
            and ur.role::text in ('church_admin', 'pastor', 'secretary', 'treasurer')
        )
        or (
