...
Members submit a payment transaction ID and optional payment proof from the member portal. The request is stored as `pending` until reviewed by a church administrator.

## Admin Review

Admins can review requests at Church Admin -> Record Preservation. Pending requests can be approved or rejected. Proof files are stored in the private `record-preservation-proofs` storage bucket and are viewed through short-lived signed URLs.
...
The `member_record_subscriptions` table stores preservation requests and active archive periods. Row level security allows members to see and create their own requests, while church admins can manage subscriptions for their church.

## Visibility Rules

Without active preservation, current month activity remains visible and historical archive rows are hidden from the member portal. With active preservation, full history and archive features are available.

## Scheduled Expiry

Run `public.expire_member_record_subscriptions()` daily so expired archive periods are marked as `expired`.
