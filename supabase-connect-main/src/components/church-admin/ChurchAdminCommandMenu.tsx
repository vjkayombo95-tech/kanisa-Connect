...
export function ChurchAdminCommandMenu() {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-xl overflow-hidden border-white/10 bg-[#0b0e13] p-0"><DialogHeader className="sr-only"><DialogTitle>Search workspace</DialogTitle><DialogDescription>Open an approved Church Admin service.</DialogDescription></DialogHeader><label className="flex items-center gap-3 border-b border-white/10 px-4"><Search className="h-5 w-5 text-muted-foreground" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search approved services…" className="h-14 flex-1 bg-transparent text-base outline-none" /></label><div className="premium-scrollbar max-h-[55vh] overflow-y-auto p-2">{results.map((service) => { const Icon = service.icon; return <button key={service.id} type="button" onClick={() => { setOpen(false); setQuery(""); navigate(service.route); }} className="flex min-h-12 w-full items-center gap-3 rounded-xl px-3 text-left text-sm text-white/70 hover:bg-white/[0.05] hover:text-white"><Icon className="h-4 w-4 text-primary" /><span className="flex-1">{service.label}</span><span className="text-xs text-white/35">{service.group}</span></button>; })}{!results.length ? <p className="px-3 py-8 text-center text-sm text-muted-foreground">No approved service matches this search.</p> : null}</div></DialogContent></Dialog>
  </>;
}
