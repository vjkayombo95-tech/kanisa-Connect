...
function formatSchedule(value: string | null, language: string | undefined, fallback: string) {
...
export function ChurchAdminLiveMediaAwareness() {
  ...
  if (loading && !livestreamAvailable && !radioAvailable) {
    return (
      <section aria-label={t("church_admin_shell.live_media.aria_label")} data-testid="church-admin-live-media-loading" className="rounded-xl border border-border/70 bg-card/85 p-5">
  ...
  return (
    <section aria-labelledby="church-admin-live-media-title" data-testid="church-admin-live-media" className="space-y-4 rounded-xl border border-primary/20 bg-card/85 p-5 shadow-sm">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{t("church_admin_shell.live_media.eyebrow")}</p>
        <h2 id="church-admin-live-media-title" className="mt-1 font-serif text-xl font-semibold text-foreground">{t("church_admin_shell.live_media.title")}</h2>
        <p className="mt-1 text-sm text-muted-foreground">{t("church_admin_shell.live_media.description")}</p>
      </div>
          ...
          <article data-testid="church-admin-livestream-awareness" className="flex min-w-0 flex-col justify-between gap-4 rounded-lg border border-border/70 bg-background/50 p-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground">{t("church_admin_shell.live_media.livestream")}</p>
                {livestream.isLoading ? <Skeleton className="mt-2 h-5 w-40" /> : livestream.isError ? (
                  <p className="mt-1 text-sm text-destructive">{t("church_admin_shell.live_media.livestream_unavailable")}</p>
                ) : livestream.data?.status === "live" ? (
                  <><p className="mt-1 text-sm font-semibold text-red-500">{t("church_admin_shell.live_media.live_now")}</p><p className="mt-1 truncate text-sm text-muted-foreground">{livestream.data.title}</p></>
                ) : livestream.data?.status === "scheduled" ? (
                  <><p className="mt-1 inline-flex items-center gap-1 text-sm font-semibold text-primary"><CalendarClock className="h-4 w-4" />{t("church_admin_shell.live_media.scheduled")}</p><p className="mt-1 text-sm text-muted-foreground">{formatSchedule(livestream.data.scheduledStart, i18n.language, t("church_admin_shell.live_media.schedule_fallback"))}</p></>
                ) : (
                  <p className="mt-1 text-sm text-muted-foreground">{t("church_admin_shell.live_media.no_broadcast")}</p>
                )}
              </div>
            </div>
            <Link to="/church-admin/livestreams" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-primary/20 px-4 text-sm font-semibold text-primary outline-none focus-visible:ring-2 focus-visible:ring-primary">
              {t("church_admin_shell.live_media.open_livestreams")} <ExternalLink className="h-4 w-4" aria-hidden="true" />
            </Link>
          </article>
        ) : null}
        ...
        {radioAvailable ? (
          <article data-testid="church-admin-radio-awareness" className="flex min-w-0 flex-col justify-between gap-4 rounded-lg border border-border/70 bg-background/50 p-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-foreground">{t("church_admin_shell.live_media.radio")}</p>
                {radio.isLoading ? <Skeleton className="mt-2 h-5 w-40" /> : radio.isError ? (
                  <p className="mt-1 text-sm text-destructive">{t("church_admin_shell.live_media.radio_unavailable")}</p>
                ) : radio.data.length ? (
                  <><p className="mt-1 text-sm font-semibold text-primary">{t("church_admin_shell.live_media.available")}</p><p className="mt-1 truncate text-sm text-muted-foreground">{radio.data.find((station) => station.isDefault)?.name ?? radio.data[0].name}</p></>
                ) : (
                  <p className="mt-1 text-sm text-muted-foreground">{t("church_admin_shell.live_media.no_station")}</p>
                )}
              </div>
            </div>
            <Link to="/church-admin/radio" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-primary/20 px-4 text-sm font-semibold text-primary outline-none focus-visible:ring-2 focus-visible:ring-primary">
              {t("church_admin_shell.live_media.open_radio")} <ExternalLink className="h-4 w-4" aria-hidden="true" />
            </Link>
          </article>
        ) : null}
      </div>
