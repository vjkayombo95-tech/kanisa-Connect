...
export function ChurchDashboardIntelligenceView({
  ...
  return (
    <section className={compact ? "grid gap-3 md:grid-cols-2" : "grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(300px,0.75fr)]"} aria-label={t("church_admin_dashboard.intelligence.aria_label")}>
      {pendingEnabled ? (
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.035] p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary/80">{t("church_admin_dashboard.intelligence.needs_attention")}</p><h2 className="mt-1 font-semibold text-white">{t("church_admin_dashboard.intelligence.pending_work")}</h2></div>
            </div>
          ...
          {pending.isLoading ? <div className="mt-4 grid gap-3 sm:grid-cols-2"><Skeleton className="h-16 rounded-xl" /><Skeleton className="h-16 rounded-xl" /></div>
            : pending.isError ? <StateMessage icon={AlertCircle} text={t("church_admin_dashboard.intelligence.pending_error")} error />
            : actions.length === 0 ? <StateMessage icon={CheckCircle2} text={t("church_admin_dashboard.intelligence.pending_empty")} />
            : <div className="mt-4 grid gap-2 sm:grid-cols-2">{actions.map((item) => <Link key={item.key} to={item.route} className="flex min-h-14 items-center justify-between gap-3 rounded-xl border border-white/[0.08] bg-black/15 px-3.5 py-3 transition hover:border-primary/25 hover:bg-primary/[0.04]"><span className="text-sm font-medium text-white">{translateSystemLabel(t, item.labelKey, item.label)}</span><span className="flex items-center gap-2 text-primary"><strong>{item.count}</strong><ChevronRight className="h-4 w-4" /></span></Link>)}</div>}
        </div>
      ) : null}
      {financialEnabled ? (
        <div className="rounded-2xl border border-primary/15 bg-primary/[0.045] p-4 sm:p-5">
          <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><CircleDollarSign className="h-5 w-5" /></span><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary/80">{t("church_admin_dashboard.intelligence.financial_summary")}</p><h2 className="mt-1 font-semibold text-white">{t("church_admin_dashboard.intelligence.verified_receipts")}</h2></div></div>
          {financial.isLoading ? <Skeleton className="mt-4 h-28 rounded-xl" />
            : financial.isError ? <StateMessage icon={AlertCircle} text={t("church_admin_dashboard.intelligence.financial_error")} error />
            : <div className="mt-4 grid grid-cols-2 gap-3"><Metric label={t("church_admin_dashboard.intelligence.this_month")} value={formatTZS(summary.thisMonthReceived)} /><Metric label={t("church_admin_dashboard.intelligence.all_received")} value={formatTZS(summary.totalReceived)} /><Metric label={t("church_admin_dashboard.intelligence.transactions")} value={String(summary.transactionCount)} /><Metric label={t("church_admin_dashboard.intelligence.contributions")} value={formatTZS(summary.contributionTotal)} /></div>}
        </div>
      ) : null}
    </section>
  );
