...
export default function FamiliesPage() {
  ...
  const displayFamilyRole = (role: string | null | undefined) =>
    role ? translateSystemLabel(t, `members_admin.family_roles.${role}`, role.replace(/_/g, " ")) : "";
  ...
  const addFamilyMember = useMutation({
    mutationFn: async () => {
      if (!detailFamily || !selectedMemberId) throw new Error(label("errors.missing_data", "Missing data"));
          ...
          <DialogContent>
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); create.mutate(); }}>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" disabled={create.isPending || !name}>{create.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {label("actions.create", "Create")}</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
            ...
            return (
              <Card key={f.id} className="glass-card hover:gold-glow transition-shadow cursor-pointer" onClick={() => setDetailFamily(f)}>
                ...
                <CardContent className="space-y-2">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground"><Users className="h-3 w-3" /> {label("cards.members_count", "{{count}} members", { count: fMembers.length })}</div>
          ...
          {detailFamily && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={() => setAddMemberOpen(true)}><UserPlus className="mr-2 h-3 w-3" /> {label("actions.add", "Add")}</Button>
              </div>
                ...
                <TableBody>
                  {getFamilyMembersList(detailFamily.id).length === 0 ? (
                    ...
                    <TableRow key={fm.id}>
                      ...
                      <TableCell><Badge variant="outline">{displayFamilyRole(fm.family_role)}</Badge></TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 text-destructive"
                          aria-label={label("accessibility.remove_member_for", "Remove {{name}} from family", { name: fm.full_name })}
                          onClick={() => removeFamilyMember.mutate(fm.id)}
                        >
                ...
                <div className="border border-border rounded-lg p-4 space-y-3 bg-secondary/30">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedRole} onValueChange={setSelectedRole}>
                      ...
                      <SelectContent>{FAMILY_ROLES.map((r) => <SelectItem key={r} value={r}>{displayFamilyRole(r)}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
