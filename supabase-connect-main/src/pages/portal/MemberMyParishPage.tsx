...
import { dailyLifeKeys, fetchLatestAnnouncement, fetchNextMass, fetchParishEvents, fetchParishIdentity, isUpcomingEvent } from "@/lib/member-daily-life";
...
export default function MemberMyParishPage() {
  ...
  return <main data-testid="member-my-parish-page" className="min-h-full bg-[linear-gradient(180deg,hsl(var(--background)),hsl(var(--muted)/0.35))] px-4 py-5 pb-28 lg:px-8 lg:pb-10"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">{mass.isLoading ? <Skeleton className="h-28 rounded-[24px]" /> : mass.data ? <LinkCard to="/portal/calendar" title={mass.data.title} detail={new Date(`${mass.data.massDate}T${mass.data.startTime}`).toLocaleString("sw-TZ", { dateStyle: "medium", timeStyle: "short" })} icon={Church} /> : null}{announcement.isLoading ? <Skeleton className="h-28 rounded-[24px]" /> : announcement.data ? <LinkCard to="/portal/announcements" title={announcement.data.title} detail={announcement.data.content || "Tangazo la karibuni"} icon={Megaphone} /> : null}</div>
